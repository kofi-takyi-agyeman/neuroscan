...
NeuroScan pairs a Flutter mobile front-end with a Python FastAPI service. Users (clinicians or technicians) can capture or upload a brain MRI image and receive a tumor classification with a confidence score, all within a clean, session-based mobile interface.
...
\_Coming soon. Save screenshots to `docs/screenshots/` and update this section.\_
...
\- A device or emulator (Android/iOS) to run the mobile app
...
Make sure your device or emulator can reach the backend URL. If running on a physical device, replace localhost in the API configuration with your machine's LAN IP.
...
\- \[ ] Take and publish mobile app screenshots
